...
#define BOE_BF060Y8M_AJ0_NUM_SUPPLIES	5
...
struct boe_bf060y8m_aj0 {
	...
	struct regulator_bulk_data vregs[BOE_BF060Y8M_AJ0_NUM_SUPPLIES];
...
static int boe_bf060y8m_aj0_prepare(struct drm_panel *panel)
{
	 ...
	 */
	ret = regulator_enable(boe->vregs[3].consumer);
	if (ret)
		return ret;
	ret = regulator_enable(boe->vregs[4].consumer);
	if (ret)
		return ret;

	ret = regulator_enable(boe->vregs[0].consumer);
	if (ret)
		return ret;
	usleep_range(1000, 2000);
	ret = regulator_enable(boe->vregs[1].consumer);
	if (ret)
		return ret;
	usleep_range(500, 1000);
	ret = regulator_enable(boe->vregs[2].consumer);
	if (ret)
		return ret;
	usleep_range(2000, 3000);
				       ...
				       struct device *dev)
{
	...
	boe->vregs[0].supply = "vcc";
	boe->vregs[1].supply = "vddio";
	boe->vregs[2].supply = "vci";
	boe->vregs[3].supply = "elvdd";
	boe->vregs[4].supply = "elvss";
	...
	ret = regulator_is_supported_voltage(boe->vregs[0].consumer,
					     2700000, 3600000);
	if (!ret)
		return ret;
	ret = regulator_is_supported_voltage(boe->vregs[1].consumer,
					     1620000, 1980000);
	if (!ret)
		return ret;
	ret = regulator_is_supported_voltage(boe->vregs[2].consumer,
					     2600000, 3600000);
	if (!ret)
		return ret;
	ret = regulator_is_supported_voltage(boe->vregs[3].consumer,
					     4400000, 4800000);
	if (!ret)
		return ret;
	/* ELVSS is negative: -5.00V to -1.40V */
	ret = regulator_is_supported_voltage(boe->vregs[4].consumer,
					     1400000, 5000000);
	if (!ret)
		return ret;
	 ...
	 */
	ret = regulator_set_current_limit(boe->vregs[1].consumer, 1500, 2500);
	if (ret)
		dev_dbg(dev, "Current limit cannot be set on %s: %d\n",
			boe->vregs[1].supply, ret);

	ret = regulator_set_current_limit(boe->vregs[2].consumer,
					  20000, 40000);
	if (ret)
		dev_dbg(dev, "Current limit cannot be set on %s: %d\n",
